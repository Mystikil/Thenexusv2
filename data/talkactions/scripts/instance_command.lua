...
function onSay(player, words, param)
    local command = words
    local args = param or ''
    if command == '!instanceid' then
        local id = parseId(args)
        if not id then
            player:sendTextMessage(MESSAGE_STATUS_SMALL, 'Usage: !instanceid <id>')
            return false
        end
    ...
    if action == 'list' or action == '' then
        local rows = ActivityManager.list(player, 'instance')
        sendLines(player, rows)
    elseif action == 'id' then
        local id = parseId(rest)
        if not id then
            player:sendTextMessage(MESSAGE_STATUS_SMALL, 'Usage: !instance id <id>')
            return false
        end
        local ok, reason = ActivityManager.enter(player, id)
        if not ok then
    ...
    elseif action == 'leave' then
        ...
        player:sendTextMessage(MESSAGE_STATUS_SMALL, 'Usage: !instance list|id <id>|info [id]|leave')
    end
    return false
end
